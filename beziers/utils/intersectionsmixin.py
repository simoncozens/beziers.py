...
class IntersectionsMixin:
    ...
    def intersections(self, other, limited=True):
        ...
        if limited:
            ...
            return inter
        raise "Don't know how to compute intersections of a %s and a %s" % (
            type(self),
            type(other),
        )
    ...
    def _line_line_intersections(self, other):
        ...
        c = other.start
        d = other.end
        if isclose(c.x, d.x) and isclose(a.x, b.x):
            return []
        if isclose(c.y, d.y) and isclose(a.y, b.y):
            return []
        if c == d or a == b:
            return []
        if isclose(b.x, a.x):
        ...
        slope12 = (b.y - a.y) / (b.x - a.x)
        slope34 = (d.y - c.y) / (d.x - c.x)
        if abs(slope12 - slope34) < my_epsilon:
            return []
        x = (slope12 * a.x - a.y - slope34 * c.x + c.y) / (slope12 - slope34)
        y = slope12 * (x - a.x) + a.y
        intersection = Point(x, y)
        if self._bothPointsAreOnSameSideOfOrigin(
            intersection, b, a
        ) and self._bothPointsAreOnSameSideOfOrigin(intersection, c, d):
            return [
                Intersection(
                    self,
                    self.tOfPoint(intersection, its_on_the_line_i_swear=True),
                    other,
                    other.tOfPoint(intersection, its_on_the_line_i_swear=True),
                )
            ]
        return []
        ...
        for t in self._curve_line_intersections_t(line):
            inter.append(
                Intersection(
                    ...
                    t,
                    line,
                    line.tOfPoint(self.pointAtTime(t), its_on_the_line_i_swear=True),
                )
            )
        return inter
    ...
    def _curve_curve_intersections(self, other):
        assert len(self.points) > 2 and len(other.points) > 2
        return [
            Intersection(self, t[0], other, t[1])
            for t in self._curve_curve_intersections_t(other)
        ]
