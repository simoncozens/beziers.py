...
class CurveFit:
    ...
    def leftTangent(self, data, tolerance):
        ...
        while True:
            ...
            if tolerance < distSq:
                return t.toUnitVector()
            i = i + 1
            if i == len(data):
                if distSq == 0:
                    return self._leftTangent(data)
                else:
                    return t.toUnitVector()
    ...
    def rightTangent(self, data, tolerance):
        ...
        while True:
            ...
            if i == 0:
                if distSq == 0:
                    return self._rightTangent(data)
                else:
                    return t.toUnitVector()
    ...
    def estimateLengths(self, data, u, tHat1, tHat2):
        ...
        return CubicBezier(
            data[0], tHat1 * alpha_l + data[0], tHat2 * alpha_r + data[-1], data[-1]
        )
    ...
    def computeMaxError(self, bez, points, params, tolerance, cornerTolerance):
        ...
        for i in range(1, len(points)):
            ...
            if distSq > maxSqDist:
                maxSqDist = distSq
                splitPoint = i
            hookRatio = self.computeHook(
                prev, cur, (params[i] + params[i - 1]) / (2.0), bez, cornerTolerance
            )
    ...
    @classmethod
    def _fitCurve(
        self, points, tangent1, tangent2, error, cornerTolerance, maxSegments
    ):
        if len(points) == 0:
            return
        if len(points) == 2:
        ...
        (maxErrorRatio, splitPoint) = self.computeMaxError(
            bez, points, u, tolerance, cornerTolerance
        )
        ...
        if 1 < maxSegments:
            ...
            if isCorner:
                if not (0 < splitPoint and splitPoint < len(points) - 1):
                    return []
            ...
            lbeziers = self._fitCurve(
                lPoints, tangent1, recTHat2, error, cornerTolerance, segmentsRemaining
            )
            if lbeziers:
                segmentsRemaining = segmentsRemaining - len(lbeziers)
            rbeziers = self._fitCurve(
                rPoints, recTHat1, tangent2, error, cornerTolerance, segmentsRemaining
            )
